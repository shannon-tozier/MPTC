...
-   
    ...
    finalplot <- ggplot(df,
      ...
      coord_flip(ylim = c(0, 1096)) +
      theme_minimal() +
      labs(title = "Length of NYC Open Complaints",
           subtitle = "By Race and Borough, 2019",
           caption = "Data sourced from the United States Census and NYC Open Data",
           y = "Days Before Complaint Was Closed \n Markers at 1 and 2 Years",
           x = "Percentage of White Population in Zip Code of Complaint") +
      guides(fill = FALSE,
             color = FALSE) +
      geom_hline(yintercept = 365, linetype = "dashed", size = 0.8) +
      geom_hline(yintercept = 730, linetype = "dashed", size = 0.8)

    ```

    a.  Try saving this plot or map using `ggsave()`. Create it as a PNG or PDF file. Make its dimensions (width and height) the right size for what you draw.
